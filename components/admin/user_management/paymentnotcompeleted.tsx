...
// ---------------------------------------------------------------------
// IncompletePaymentMembersTable Component
// ---------------------------------------------------------------------
export default function IncompletePaymentMembersTable() {
  const dispatch = useDispatch<AppDispatch>();
  const router = useRouter();
  const { incompletePaymentMembers } = useSelector((state: RootState) => state.members);
  ...
  const filteredIncompleteMembers = useMemo(() => {
    return incompletePaymentMembers
      .filter((member) => {
        // Combine member's first and last name, email, and phone for searching
        const fullName = `${member.first_name} ${member.last_name}`.toLowerCase();
        const searchFields = [fullName, member.email, member.phone].join(" ").toLowerCase();

        // Payment filter: if member is not fully paid, they match the "unpaid" condition.
        // Adjust logic as necessary if you need a different check.
  ...
  }, [incompletePaymentMembers, searchTerm, amountFilter, planFilter, order, orderBy]);
  ...
  const handleBlockMember = (id: number) => {
    // Optionally implement block functionality for incomplete payment members if needed
    ...
    <CssVarsProvider theme={darkTheme} defaultMode="dark">
      <Sheet sx={{ p: 2, borderRadius: "sm", bgcolor: "transparent" }}>
        ...
        >
          <Input
            size="sm"
            placeholder="Search incomplete members..."
        ...
        {/* Incomplete Payment Members Table */}
        <Sheet
          variant="outlined"
          sx={{
            width: "100%",
        ...
        >
          <Typography level="body-sm" sx={{ color: "rgba(255,255,255,0.7)" }}>
            Showing {Math.min(filteredIncompleteMembers.length, (page + 1) * rowsPerPage)} of {filteredIncompleteMembers.length} members
          </Typography>
          <Box sx={{ display: "flex", gap: 1 }}>
            ...
            <Box sx={{ display: { xs: "none", md: "flex" }, gap: 1 }}>
              {Array.from({ length: Math.ceil(filteredIncompleteMembers.length / rowsPerPage) }, (_, i) => (
                <IconButton
                  key={i}
                  size="sm"
            ...
            <Button
              ...
              disabled={page >= Math.ceil(filteredIncompleteMembers.length / rowsPerPage) - 1}
              onClick={() => setPage(page + 1)}
              sx={{
                color: "#fff",
                borderColor: "rgba(255,255,255,0.3)",
