...
import { fetchMembers, Member } from "@/src/features/members/memberSlice";
import {
  Avatar,
  Box,
  Chip,
...
// Define the allowed membership statuses
type MembershipStatus = "active" | "expired" | "blocked" | "expiring" | "payment_due";

// Create a mapping for chip properties
...
> = {
  ...
  expiring: {
    ...
    textColor: "#ffcc80",
    icon: <AccessTimeIcon />,  // or any icon that signifies expiring
  },
  payment_due: {
    color: "primary",
    bgColor: "rgba(60, 60, 255, 0.3)",
    textColor: "#80a0ff",
    icon: <PaymentIcon />, // Use an appropriate payment icon
  },
};
...
export default function MemberTable() {
  ...
  // Render filters for status, payment, and plan
  const renderFilters = () => (
    <>
      <FormControl size="sm">
        ...
        <Select
          ...
          value={statusFilter}
          onChange={(e) =>
            e && setStatusFilter((e.target as HTMLSelectElement).value)
          }
      ...
      <FormControl size="sm">
        ...
        <Select
          ...
          value={amountFilter}
          onChange={(e) =>
            e && setAmountFilter((e.target as HTMLSelectElement).value)
          }
      ...
      <FormControl size="sm">
        ...
        <Select
          ...
          value={planFilter}
          onChange={(e) =>
            e && setPlanFilter((e.target as HTMLSelectElement).value)
          }
      ...
      >
                ...
                <tr key={member.id}>
                  ...
                  <td>
                    <Chip
                      ...
                      size="sm"
                      startDecorator={
                        statusMapping[member.membership_status as MembershipStatus].icon
                      }
                      sx={{
                      ...
                      <Tooltip title="More Actions">
                        <Dropdown>
                          ...
                          >
                            ...
                            <MenuItem
                              onClick={() =>
                                router.push(`/admin/membermanagement/payments/${member.id}`)
                              }
                            >
                              Payment History
                            </MenuItem>
                            <MenuItem
                              onClick={() =>
                                router.push(`/admin/membermanagement/renewal/${member.id}`)
                              }
                            >
                              Renewal
                            </MenuItem>
        ...
        <Box sx={{ display: { xs: "block", md: "none" } }}>
          {filteredMembers.map((member) => (
            <Card key={member.id} sx={{ my: 1 }}>
              <CardContent>
                <Typography level="body-sm">
                  #{member.id} - {member.first_name} {member.last_name}
                </Typography>
                <Typography level="body-xs">{member.phone}</Typography>
                <Typography level="body-xs">₹{member.amount_paid}</Typography>
                <Chip size="sm">{member.membership_status}</Chip>
